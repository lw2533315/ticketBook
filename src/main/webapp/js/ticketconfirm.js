...
// $("#passPrice").val(price);
		...
		for(let i = orgSeats; i < totalSeats; i++){
			...
			let row1 = "<h5 style='color:#3333CC; font-size:22px' class='block"+(i+1)+"' >the "+ (i+1)+"st passenger</5>"
			header.before(row1);
			
			let row2 = "<div class='block"+(i+1)+"'><p class='cofirminfo'>First Name</p><input type='text' name ='firstName' value=''/></div>"
			let row3 = "<div class='block"+(i+1)+"'><p class='cofirminfo'>Last Name</p><input type='text' name ='lastName' value=''/></div>"
			header.before(row2);
			header.before(row3);
			
			let row4 = "<div class='block"+(i+1)+"'><p class='cofirminfo'>Email</p><input type='text' name ='email' value=''/></div>"
			let row5 = "<div class='block"+(i+1)+"'><p class='cofirminfo'>Phone</p><input type='text' name ='phone' value=''/></div>"	
			header.before(row4);
			header.before(row5);
			
			let row6 = "<div class='block"+(i+1)+"'><p class='cofirminfo'>Gender</p> <select name = 'gender' style='width: 165px; color:black; font-size: 15px' >"
                                    ...
                                    +"</select> </div>"
			let row7 = "<div class='block"+(i+1)+"'><p class='cofirminfo'>Age</p><input type='text' name ='age' value=''/></div>"
			header.before(row6);
			header.before(row7);
		}
